...
from ...schemas.statistic import GetStatistic


async def delete_statistic_table() -> None:
    async with AsyncSession(engine) as session:
        async with session.begin():
            await session.execute(sa.delete(Statistic))


async def update_statistic_row(row: dict):
    async with AsyncSession(engine) as session:
        async with session.begin():
            pre_commit = await session.execute(
                sa.select(Statistic).where(Statistic.date == row["date"])
            )
            if pre_commit.scalars().first():
                date = row.pop("date")
                await session.execute(
                    sa.update(Statistic).where(Statistic.date == date).values(row)
                )
            else:
                await session.execute(sa.insert(Statistic).values(row))


async def select_statistic(params: GetStatistic) -> dict:
    async with AsyncSession(engine) as session:
        async with session.begin():
            query = sa.select(
                Statistic.date,
